...
int Dice::roll(int times, int sides, int div = 1) {
	int result = 0;
	for (int i=1; i<=times; i++) result += random(sides);
...
string Dice::check_result(int result, int skill, int type) {
	...
	if (type == 0) {
		if (result > skill) {
			if ((skill < 50 && result > 95) || result == 100) r << "大失败";
			else r << "失败";
		} else if (half < result && result <= skill) r << "普通成功";
		else if (fifth < result && result <= half) r << "困难成功";
		else if (result <= fifth && result > 1) r << "极难成功";
		else if (result == 1) r << "大成功";
	} else if (type == 1) {
		if (result > 95 && result > skill) r << "大失败";
		else if (result <= 5 && skill > result) r << "大成功";
		else if (skill < result && result <= 95) r << "失败";
		else if (half < result && result <= skill) r << "普通成功";
		else if (fifth < result && result <= half) r << "困难成功";
		else if (result <= fifth) r << "极难成功";
	} else if (type == 2) {
		if (result > skill || result > 95) r << "成功";
		else r << "失败";
	}
	return r.str();
}
...
string Dice::toss(string msg, int type, int &s) {
	...
	if (type == 0) {
		...
		if (regex_search(msg, m_nn, num_num)) {
			...
			n = stoi(extract(temp, 0, 'd'));
			d = 100;
		} else if (regex_search(msg, m_dd, default_default) || msg == ">h") d = 100;
		...
		if (regex_search(msg, m_nn, num_num)) {
			...
			n = stoi(extract(temp, 0, 'd'));
			d = 100;
		} else if (regex_search(msg, m_dd, default_default)) d = 100;
		...
		if (n > 50) throw n;
		if (type == 1) result << n << "d" << d;
		else result << Dice::roll(n, d, div, false, s);
	}
