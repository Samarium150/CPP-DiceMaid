...
string Maid::get_help() {
    stringstream help;
    help << "帮助文档: " << endl;
	help << "1. >help: 查看帮助文档;" << endl;
	help << "2. >jrrp: 查看今日人品;" << endl;
	help << "3. >r骰子表达式: 投一个指定的骰子, 不写骰子个数则默认为1;默认>rd=r1d100;" << endl;
	help << "4. >ra [检定项目] [数值]: 进行一次指定技能的检定, 命令、项目和技能数值之间必须要用空格隔开, 示例: >ra 侦察 50;" << endl;
	help <<	"5. >sc [San值表达式] [当前San值]: 快捷San check, 示例: >sc 1d10/1d100 80;" << endl;
	help << "6. >h骰子表达式: 进行一次暗骰, 默认为1d100;" << endl;
	help <<	"7. >tarot [理由]: 抽一张塔罗牌;" << endl;
	help << "8. >ti/li: 决定疯狂症状;" << endl;
	help <<	"9. >coc7 [正整数n]：生成n组7版人物数据，最多10组;" << endl;
	help <<	"10. >en [检定项目] [数值]: 进行一次指定技能的成长检定, 命令、项目和技能数值之间必须要用空格隔开, 示例: >en 侦察 50;" << endl;
	help <<	"11. >rb[正整数n] [检定项目] [数值]: 进行一次附加一个/n个奖励骰的指定技能的检定;" << endl;
	help <<	"12. >rp[正整数n] [检定项目] [数值]: 进行一次附加一个/n个惩罚骰的指定技能的检定;" << endl;
	help << "13. >set rate 0/1: 设置技能检定倍率（有无倍增），默认为规则书标准倍率" << endl;
	help << "14. >!dissmiss 骰娘QQ号: 让骰娘退群";
    return help.str();
}
...
string Maid::command(long id, string msg) {
    ...
    if (regex_match(msg, m_ra, ra)) return Dice::check(reason, num, rate);
	else if (regex_match(msg, m_en, en)) return Dice::check(reason, num, rate, 2);
    else if (regex_match(msg, m_rb, rb)) return Dice::check(reason, num, rate, 3, count);
    else if (regex_match(msg, m_rp, rp)) return Dice::check(reason, num, rate, 4, count);
